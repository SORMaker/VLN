...
def get_release_scans(release_file):
    response = requests.get(release_file)
    response.raise_for_status()  # Raise an exception for bad status codes
    # Split the text into lines and strip whitespace
    scans = [line.strip() for line in response.text.splitlines()]
    return scans
...
def download_file(url, out_file):
    ...
    if not os.path.isfile(out_file):
        ...
        os.close(fh)  # Close the file handle immediately
        
        # Use wget to download the file
        try:
            # subprocess.run(['wget', '-q', url, '-O', out_file_tmp], check=True)
            subprocess.run(['wget', '--quiet', '--show-progress', '--continue', url, '-O', out_file_tmp], check=True)
